...
function Invoke-IcingaCheckPartitionSpace()
{
    ...
    foreach ($partition in $Disks.Keys) {
        ...
        $IcingaCheck   = $null;
        $PartitionName = ([string]::Format('Partition {0}', $partition.DriveLetter));

        if ([string]::IsNullOrEmpty($partition.VolumeName) -eq $FALSE) {
            $PartitionName = $PartitionName = ([string]::Format('Partition {0} ({1})', $partition.DriveLetter, $partition.VolumeName));
        }

        if ($CheckUsedSpace) {
            $IcingaCheck = New-IcingaCheck -Name $PartitionName -Value $partition.UsedSpace -Unit 'B' -Minimum 0 -Maximum $partition.Size -LabelName ([string]::Format('used_space_partition_{0}', $FormattedLetter)) -NoPerfData:$SetUnknown -BaseValue $partition.Size;
        } else {
            $IcingaCheck = New-IcingaCheck -Name $PartitionName -Value $partition.FreeSpace -Unit 'B' -Minimum 0 -Maximum $partition.Size -LabelName ([string]::Format('free_space_partition_{0}', $FormattedLetter)) -NoPerfData:$SetUnknown -BaseValue $partition.Size;
        }
